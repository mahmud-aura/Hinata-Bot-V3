...
module.exports = {
        config: {
                ...
                role: 0,
                description: {
                        en: "Give someone a trigger effect",
                        vi: "Tạo hiệu ứng trigger cho ai đó"
                },
                category: "fun",
                guide: {
                        en: '   {pn} <@tag>: Give trigger effect by tagging'
                                + '\n   {pn} <uid>: Create effect using UID'
                                + '\n   (Or use by replying to a message)',
                        vi: '   {pn} <@tag>: Tạo hiệu ứng trigger bằng cách gắn thẻ'
        ...
        onStart: async function ({ api, event, args, message, getLang }) {
                ...
                const filePath = path.join(cacheDir, `trigger_${id2}.gif`);

                try {
                        if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir);

                        const response = await axios.get(`${await mahmud()}/api/fun?type=trigger&user=${id2}`, { responseType: "arraybuffer" });
