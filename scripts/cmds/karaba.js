...
module.exports = {
        config: {
                ...
                role: 0,
                description: {
                        en: "Give someone a karaba effect",
                        vi: "Tạo hiệu ứng karaba cho ai đó"
                },
                category: "fun",
                guide: {
                        en: '   {pn} <@tag>: Give karaba effect by tagging'
                                + '\n   {pn} <uid>: Create effect using UID'
                                + '\n   (Or use by replying to a message)',
                        vi: '   {pn} <@tag>: Tạo hiệu ứng karaba bằng cách gắn thẻ'
        ...
        onStart: async function ({ api, event, args, message, getLang }) {
                ...
                const filePath = path.join(cacheDir, `karaba_${id2}.png`);

                try {
                        if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir);

                        const response = await axios.get(`${await mahmud()}/api/fun?type=karaba&user=${id2}`, { responseType: "arraybuffer" });
