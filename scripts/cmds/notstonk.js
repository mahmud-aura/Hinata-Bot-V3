...
module.exports = {
        config: {
                ...
                role: 0,
                description: {
                        en: "Give someone a notstonk effect",
                        vi: "Tạo hiệu ứng notstonk cho ai đó"
                },
                category: "fun",
                guide: {
                        en: '   {pn} <@tag>: Give notstonk effect by tagging'
                                + '\n   {pn} <uid>: Create effect using UID'
                                + '\n   (Or use by replying to a message)',
                        vi: '   {pn} <@tag>: Tạo hiệu ứng notstonk bằng cách gắn thẻ'
        ...
        onStart: async function ({ api, event, args, message, getLang }) {
                ...
                const filePath = path.join(cacheDir, `notstonk_${id2}.png`);

                try {
                        if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir);

                        const response = await axios.get(`${await mahmud()}/api/fun?type=notstonk&user=${id2}`, { responseType: "arraybuffer" });
