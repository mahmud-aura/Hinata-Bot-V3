...
module.exports = {
        config: {
                ...
                role: 0,
                description: {
                        en: "Give someone an elephant effect",
                        vi: "Tạo hiệu ứng elephant cho ai đó"
                },
                category: "fun",
                guide: {
                        en: '   {pn} <@tag>: Give elephant effect by tagging'
                                + '\n   {pn} <uid>: Create effect using UID'
                                + '\n   (Or use by replying to a message)',
                        vi: '   {pn} <@tag>: Tạo hiệu ứng elephant bằng cách gắn thẻ'
        ...
        onStart: async function ({ api, event, args, message, getLang }) {
                ...
                const filePath = path.join(cacheDir, `elephant_${id2}.png`);

                try {
                        if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir);

                        const response = await axios.get(`${await mahmud()}/api/fun?type=elephant&user=${id2}`, { responseType: "arraybuffer" });
