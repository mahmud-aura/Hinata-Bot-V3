...
module.exports = {
        config: {
                ...
                role: 0,
                description: {
                        en: "Give someone a delete effect",
                        vi: "Tạo hiệu ứng delete cho ai đó"
                },
                category: "fun",
                guide: {
                        en: '   {pn} <@tag>: Give delete effect by tagging'
                                + '\n   {pn} <uid>: Create effect using UID'
                                + '\n   (Or use by replying to a message)',
                        vi: '   {pn} <@tag>: Tạo hiệu ứng delete bằng cách gắn thẻ'
        ...
        onStart: async function ({ api, event, args, message, getLang }) {
                ...
                const filePath = path.join(cacheDir, `delete_${id2}.png`);

                try {
                        if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir);

                        const response = await axios.get(`${await mahmud()}/api/fun?type=delete&user=${id2}`, { responseType: "arraybuffer" });
